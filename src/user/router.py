from fastapi import APIRouter, Depends, status, Form, HTTPException
from fastapi.security import OAuth2PasswordBearer, OAuth2PasswordRequestForm
from sqlalchemy.ext.asyncio import AsyncSession

from database import get_async_session
from user.schemas import UserCreateSchema, UserReadSchema, AccessTokenSchema
from .jwt_repository import CredentialsRepository
from .repository import Repository
from .utils import make_token

router = APIRouter(tags=["users"], prefix="/users")
get_token = OAuth2PasswordBearer(tokenUrl="/users/login")


@router.post("/register", status_code=status.HTTP_201_CREATED)
async def register_user(login: str = Form(), password: str = Form(), password_repeat = Form(),
                        session: AsyncSession = Depends(get_async_session),
                   repository: Repository = Depends()) -> None:
    user = UserCreateSchema(nickname = login, password = password, data = "")
    if password == password_repeat:
        await repository.create_user(user, session)
    else:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="нет такой страницы")



@router.get("/", status_code=status.HTTP_200_OK)
async def get_users(repository: Repository = Depends(),
                    session = Depends(get_async_session)) -> list[UserReadSchema]:
    return await repository.get_users(session)


@router.post("/login")
async def login_user(login: str = Form(), password: str = Form(), 
                session: AsyncSession = Depends(get_async_session),
                repository_user: Repository = Depends()):
    print(login, password)
    user = await repository_user.get_user(login, password, session)
    token = make_token(user.id)
    return AccessTokenSchema(
        access_token=token,
        token_type="Bearer"
    )
